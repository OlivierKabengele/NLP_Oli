...
class BERTEncoder(nn.Module):
    """Simple BERT-style encoder for masked LM. Uses an extra MASK token id appended to vocab.

    Note: This implementation increases vocab_size by 1 to include MASK token (default id = vocab_size-1).
    """
...
class FeatureComparator(nn.Module):
    """Compare a query embedding to four category embeddings across 3 features.

    Produces final logits over 4 category slots and also per-feature similarities and gate logits.
    """
    ...
    def forward(self, cards_sub_emb: torch.Tensor, q_sub_emb: torch.Tensor):
        ...
        sims = []
        for f in range(3):
            # take the f-th subtoken for each card: (B,4,D)
            c_f = cards_sub_emb[:, :, f, :]
            q_f = q_sub_emb[:, f, :]
            ...
            sims.append(sim_f)
        sims = torch.stack(sims, dim=1)  # (B,3,4)
        # build gate input from query mean and card summary
        cards_mean = cards_sub_emb.mean(dim=(1, 2))  # (B, D)
        q_mean = q_sub_emb.mean(dim=1)  # (B, D)
        # small learned probe for signature
        # create signature by projecting each card subtoken set and taking mean abs
        # reuse query_proj[0] as a small linear for signature (or define separate if needed)
        # we compute a simple signature: mean abs over cards of the sum of subtoken projections
