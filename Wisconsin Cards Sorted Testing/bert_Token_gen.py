...
class WCST:
    ...
    def __init__(self, batch_size, switch_prob=0.7):
        ...
        # probability of switching the active feature at each generated batch
    ...
    def context_switch(self): #self.category_feature = np.random.choice(np.delete([0,1,2],self.category_feature))
        prev = self.category_feature
        choices = [f for f in [0, 1, 2] if f != self.category_feature]
        self.category_feature = int(np.random.choice(choices)) # mAKING sure that it's a Python integer (not a NumPy scalar).
        #print(f"Context switch: feature {prev} -> {self.category_feature}")

    def gen_batch(self):
        batch_size = self.batch_size
        while True: # randomly decide to switch active feature before producing this batch
