...
from bert_SubToken_gen import WCSTBertDataset, COLOR_BASE, SHAPE_BASE, NUMBER_BASE
...
def compute_roc_auc(loader, encoder, comparator, device):#Compute multiclass ROC AUC (macro, OVR) using sklearn 
    try:
        from sklearn.metrics import roc_auc_score
    except Exception:
        return None

# INTERPRETABILITY:Analyze attention maps to find heads that attend from query subtokens to correct card feature subtokens.
# KINDLY REPLACE THE PATH OF out_prefix='/home/olik/NLP/Project_NLP/interpret
def analyze_interpretability(loader, encoder, comparator, device, max_batches=20, out_prefix='/home/olik/NLP/Project_NLP/interpret'):
    ...
    try:
        for b_ix, (x, y, m, feat, lbl) in enumerate(loader):
            if b_ix >= max_batches:
                break
            x = x.to(device)
            m = m.to(device)            # request attention maps and hidden states
            with torch.no_grad():  # when return_attn and return_hidden both True, encoder returns (logits, attn_maps, hidden)
                ...
                n_heads = attn_maps[0].size(1)            # init accumulators
                for f in range(3):
                    if acc[f] is None:
                ...
                for i in range(B):
                    ...
                    for f in range(3):
                        pos_q = 4 * 3 + f
                        pos_c = 3 * lab + f
                        for L in range(n_layers):                    # attn_maps[L]: (B, n_heads, T, T)
    ...
    y_trues = np.concatenate(y_trues, axis=0)     # build one-hot ground truth
    try:
...
if __name__ == '__main__':    # small training run to exercise model + plot loss
    ...
    epochs = 29    # history tracks losses and category accuracies per epoch
    history = {
        'train_loss': [],
        'val_loss': [],
        'train_cat_acc': [],
    ...
    best_path = '/home/olik/NLP/Project_NLP/best_encoder.pt' # # KINDLY CHANGE THE PATH
    ...
    for epoch in range(1, epochs + 1):
        ...
        history['val_loss'].append(val_loss) # store category accuracy (not LM accuracy) for training/validation
        ...
        try:
            if hasattr(full_ds, 'w') and full_ds.w is not None:
                try:
                    ctx_batch, q_batch = next(full_ds.w.gen_batch())    # take first trial in the batch
                    seq = ctx_batch[0].astype(int).tolist() # decode human-readable cards as WCSTBertDataset
                    colours = ['red', 'blue', 'green', 'yellow']
                    shapes = ['circle', 'square', 'star', 'cross']
                    quantities = ['1', '2', '3', '4']
                    decoded = []
                    ...
                    with torch.no_grad():
                        ...
                        try:
                            ...
                            true_label = None          # classification loss for this sample
                        try:
        ...
        np.save(f'/home/olik/NLP/Project_NLP/val_conf_cat_epoch{epoch}.npy', val_conf_cat.numpy()) # KINDLY CHANGE THE PATH
        np.save(f'/home/olik/NLP/Project_NLP/val_conf_feat_epoch{epoch}.npy', val_conf_feat.numpy()) # KINDLY CHANGE THE PATH
        try:
            with open(f'/home/olik/NLP/Project_NLP/val_mispred_epoch{epoch}.json', 'w') as f:  # KINDLY CHANGE THE PATH
                json.dump(val_misp, f, indent=2)
        except Exception:
            pass

        # CONFUSION MATRIX computation (precision/recall/F1) for category predictions
        try:
    ...
    np.save('/home/olik/NLP/Project_NLP/test_conf_cat.npy', test_conf_cat.numpy())  # KINDLY CHANGE THE PATH
    np.save('/home/olik/NLP/Project_NLP/test_conf_feat.npy', test_conf_feat.numpy())  # KINDLY CHANGE THE PATH
    try:
        with open('/home/olik/NLP/Project_NLP/test_mispred.json', 'w') as f:    # KINDLY CHANGE THE PATH
    ...
    plt.title('Accuracy')
    plt.tight_layout()
    out_path = '/home/olik/NLP/Project_NLP/train_loss.png'   # KINDLY CHANGE THE PATH
...
if __name__ == '__main__':
    ...
    try:
        with open('/home/olik/NLP/Project_NLP/metrics.json', 'w') as f:   # KINDLY CHANGE THE PATH
        ...
        print('Saved metrics summary to /home/olik/NLP/Project_NLP/metrics.json')  # KINDLY CHANGE THE PATH
    except Exception:
        pass
    ...
    try:
        ...
        for e in last_idxs:
            try:
                arr = np.load(f'/home/olik/NLP/Project_NLP/val_conf_cat_epoch{e}.npy')   # KINDLY CHANGE THE PATH
                print(f' Epoch {e}:')
                print(arr)
    ...
    try:
        ...
        if head_scores is not None:
            for f in head_scores:   # KINDLY CHANGE THE PATH BEFORE PRINTING
