...
class WCST:
    ...
    def context_switch(self):
        ...
        # choose a different feature uniformly at random
        choices = [f for f in [0, 1, 2] if f != self.category_feature]
        self.category_feature = int(np.random.choice(choices))
        print(f"Context switch: feature {prev} -> {self.category_feature}")

    def gen_batch(self):
        batch_size = self.batch_size
        while True:
            # randomly decide to switch active feature before producing this batch
